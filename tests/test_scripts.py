...
class ScriptTests(unittest.TestCase):
    ...
    def test_audio_lines_single_source_reuse_script_is_safe_and_parallel(self) -> None:
        ...
        self.assertIn("run_annotation_with_retries", script)
        self.assertIn("--max-terminal-failures", script)
        self.assertIn("--terminal-failures-output-path", script)
        self.assertIn("terminal_annotation_failures.jsonl", script)
        self.assertIn("failures are quarantined, never fabricated", script)
        ...
        self.assertIn('local original_fail_on_transient="$FAIL_ON_TRANSIENT_OMNI_ERRORS"', script)
        self.assertIn('if [ "$attempt" -gt 1 ]; then', script)
        self.assertIn("FAIL_ON_TRANSIENT_OMNI_ERRORS=0", script)
        self.assertIn('FAIL_ON_TRANSIENT_OMNI_ERRORS="$original_fail_on_transient"', script)
    ...
    def test_audio_cvr_v1_b_first_scripts_use_10s_clips_and_keep_all_b(self) -> None:
        ...
        self.assertIn("--short-clip-group-dataset", clip_script)
        ...
        self.assertIn("--max-terminal-annotation-failures", run_script)
    ...
    def test_avatar_like_test1000_launcher_reuses_existing_four_gpu_service(self) -> None:
        ...
        self.assertIn("PROPOSE_PARALLEL_JOBS=${PROPOSE_PARALLEL_JOBS:-32}", script)
        self.assertIn("CONCURRENCY=${CONCURRENCY:-24}", script)
        self.assertIn("MAX_TERMINAL_ANNOTATION_FAILURES=${MAX_TERMINAL_ANNOTATION_FAILURES:-5}", script)
        self.assertIn('--max-terminal-annotation-failures "$MAX_TERMINAL_ANNOTATION_FAILURES"', script)
        ...
        self.assertIn("PILOT_GROUPS_PER_DATASET=${PILOT_GROUPS_PER_DATASET:-300}", script)
        self.assertIn("PILOT_SECOND_GROUPS_PER_DATASET=${PILOT_SECOND_GROUPS_PER_DATASET:-600}", script)
        ...
        self.assertIn("source_ingest_cached", script)
        self.assertIn("newly downloaded media will not be injected into this run", script)
        self.assertIn("clip_build_cached", script)
        self.assertIn("durable == segments", script)
    ...
    def test_audio_cvr_v1_b_first_4gpu_fast_starts_omni_and_uses_b_first_pipeline(self) -> None:
        ...
        self.assertIn("MAX_SOURCE_VIDEOS=${MAX_SOURCE_VIDEOS:-0}", script)
        self.assertIn("--max-source-videos N", script)
        self.assertIn("--max-source-videos \"$MAX_SOURCE_VIDEOS\"", script)
        self.assertIn('--max-source-folders) MAX_SOURCE_FOLDERS="$2"', script)
        self.assertIn('--max-clips) MAX_CLIPS="$2"', script)
        self.assertIn('--max-b-candidates) MAX_B_CANDIDATES="$2"', script)
        self.assertIn("PROPOSE_PARALLEL_JOBS=${PROPOSE_PARALLEL_JOBS:-8}", script)
        self.assertIn("CONCURRENCY=${CONCURRENCY:-4}", script)
    ...
    def test_audio_cvr_6_9s_full_scripts_are_bline_handoff_ready(self) -> None:
        ...
        self.assertIn("SHORT_CLIP_GROUP_DATASETS=${SHORT_CLIP_GROUP_DATASETS:-voxceleb}", clip_script)
        self.assertIn("--dataset-video-root hdtf=videos", clip_script)
        self.assertIn("--dataset-video-root voxceleb=vox2_mp4/dev", clip_script)
        self.assertIn("--short-clip-group-dataset \"$dataset\"", clip_script)
    ...
    def test_e5_audio_delta_train_scripts_are_smoke_only(self) -> None:
        ...
        self.assertIn("--device) DEVICE=\"$2\"; shift 2 ;;", smoke_script)
        self.assertIn("--local-segments) LOCAL_SEGMENTS=\"$2\"; shift 2 ;;", smoke_script)
        self.assertIn("--local-segment-mode) LOCAL_SEGMENT_MODE=\"$2\"; shift 2 ;;", smoke_script)
        self.assertIn("--training-profile) TRAINING_PROFILE=\"$2\"; shift 2 ;;", smoke_script)
        ...
        self.assertIn("--local-segments \"$LOCAL_SEGMENTS\"", smoke_script)
        self.assertIn("--local-segment-mode \"$LOCAL_SEGMENT_MODE\"", smoke_script)
        self.assertIn("python3 -m app.e5_audio_delta_train train-adapter", smoke_script)
        self.assertIn("--training-profile \"$TRAINING_PROFILE\"", smoke_script)
        self.assertIn("python3 -m app.e5_audio_delta_train eval", smoke_script)
        self.assertIn("--mock-encoder", smoke_script)
        self.assertIn("has_audio_delta_records()", smoke_script)
        self.assertIn("--synthetic-smoke", smoke_script)
        self.assertIn("synthetic_dataset_run", smoke_script)
        self.assertIn("audio_cvr_bline_6_9s_full_", smoke_script)
        self.assertIn("audio_cvr_ab_6_9s_minimal_", smoke_script)
        self.assertNotIn("audio_ab_fresh800_omni_first_", smoke_script)
    ...
    def test_audio_cvr_fewshot_bidir_launcher_freezes_test_and_cleans_exact_omni_group(self) -> None:
        ...
        self.assertIn('--media-root "$MEDIA_ROOT"', script)
