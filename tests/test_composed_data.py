...
class ComposedDataTests(unittest.TestCase):
    ...
    def test_annotate_clips_routes_each_clip_stably_across_endpoint_pool(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ensure_layout(root)
            clip_ids = [f"clip_{index:02d}" for index in range(12)]
            ...
            self._write_jsonl(
                ...
                [
                    {
                        "clip_id": clip_id,
                        "source_asset_id": f"asset_{clip_id}",
            ...
            for clip_id in clip_ids:
                ...
                expected[clip_id] = endpoints[int.from_bytes(digest[:8], byteorder="big") % len(endpoints)]
    ...
    def test_concurrent_annotation_persists_successes_before_retrying_failed_clip(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with mock.patch("app.composed_data.OpenAIComposedDataClient", FailOneClient):
                with self.assertRaisesRegex(RuntimeError, "successful concurrent annotations were persisted"):
                    annotate_clips(
                        root=root,
                        clips_manifest_path=manifest_path,
                        output_path=output_path,
            ...
            partial = [json.loads(line) for line in output_path.read_text(encoding="utf-8").splitlines() if line.strip()]
            self.assertEqual({"clip_a", "clip_c"}, {record["clip_id"] for record in partial})

            class SuccessClient(FailOneClient):
                def annotate_clip(self, *, clip_path: str) -> tuple[dict[str, object], dict[str, object]]:
                    return annotation_payload(Path(clip_path).stem)

            with mock.patch("app.composed_data.OpenAIComposedDataClient", SuccessClient):
                summary = annotate_clips(
                    root=root,
                    clips_manifest_path=manifest_path,
                    output_path=output_path,
            ...
            self.assertEqual(2, summary["reused_count"])
    ...
    def test_propose_group_pairs_persists_progress_before_interrupt(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ensure_layout(root)
            clip_names = ("clip_ref.mp4", "clip_target.mp4", "clip_neg1.mp4", "clip_neg2.mp4")
            ...
            self._write_jsonl(
                ...
                [
                    ...
                    },
                    {
                        "clip_id": "clip_neg1",
                        "output_path": "clips/clip_neg1.mp4",
                        "summary": "one orange cat stretching on a sofa",
                        "subjects": ["cat"],
                        "object_counts": {"cat": 1},
                        "actions": ["stretching"],
                        "scene": "living room",
                ...
                return (
                    {
                        ...
                        "proposal_reason": "same room with a count change",
                    },
                    {"provider": "mock"},
                )
