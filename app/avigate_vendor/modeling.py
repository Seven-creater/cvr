...
from .module_cross import CrossModel, CrossConfig, Transformer_Text, Transformer_Gate
            ...
            if contain_frame_position is False:
                for key, val in clip_state_dict.items():
                    ...
                    if model.sim_header == "seqTransf" and key.find("transformer.resblocks") == 0:
                        ...
                        # cut from beginning
                        if num_layer < task_config.cross_num_hidden_layers:
                            state_dict[key.replace("transformer.", "transformerClip.")] = val.clone()   ### transformerClip denotes Gated_Fusion_Transformer in the paper ###
                        if num_layer < task_config.audio_query_layers:
                            state_dict[key.replace("transformer.", "transformer_Fusion.")] = val.clone() ### transformer_Fusion denotes Audio_Resampler in the paper ###
                            continue
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.lambda_ = task_config.temperature
        ...
        """self.sim_header is only seqTransf in our project """
        if self.sim_header == "seqTransf":
            ### transformer_Fusion denotes Audio_Resampler in the paper. ###
            self.transformer_Fusion = Transformer_Text(width=transformer_width, layers=self.task_config.audio_query_layers,
                                                   heads=transformer_heads, )            
            ### transformerClip denotes Gated_Fusion_Transformer in the paper. ###
            self.transformerClip = Transformer_Gate(width=transformer_width, layers=self.task_config.cross_num_hidden_layers,
                                                   heads=transformer_heads, )
        ### Not used ###
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None, audio=None):
        ...
        audio_output = self.get_audio_output(audio)

        if self.training:
            loss = 0.
            sim_matrix, *_tmp = self.get_similarity_logits(sequence_output, visual_output, audio_output, attention_mask, video_mask,
                                                    shaped=True, loose_type=self.loose_type)

            sim_loss1 = self.loss_fct_vis(sim_matrix)
            sim_loss2 = self.loss_fct_vis(sim_matrix.T)
            sim_loss = (sim_loss1 + sim_loss2) /2
            loss += sim_loss


            return loss#, vis_feats
        else:
            return None
    ...
    def _max_pooling_for_similarity_visual(self, visual_output, video_mask,):
        ...
        video_mask_un_sum = torch.sum(video_mask_un, dim=1, dtype=torch.float)
        video_mask_un_sum[video_mask_un_sum == 0.] = 1.
        video_out = torch.max(visual_output, dim=1)[0]
        return video_out

    def _mean_pooling_for_similarity_visual_group(self, visual_output, video_mask,):
        video_mask_un = video_mask.to(dtype=torch.float).unsqueeze(-1)
        visual_output = visual_output * video_mask_un
        n_groups = 16
        itv_ = int(visual_output.shape[1]/n_groups)
        video_out = torch.zeros(visual_output.shape[0], n_groups, visual_output.shape[2]).cuda(device=visual_output.device)
        for i in range(n_groups):
            video_mask_un_sum = torch.sum(video_mask_un[:,itv_*i:itv_*(i+1),:], dim=1, dtype=torch.float)
            video_mask_un_sum[video_mask_un_sum == 0.] = 1.
            video_out[:,i,:] = torch.sum(visual_output[:,itv_*i:itv_*(i+1),:], dim=1) / video_mask_un_sum
        return video_out
    ...
    def _loose_similarity_audio_group(self, sequence_output, visual_output, audio_output, attention_mask, video_mask, sim_header="meanP"):
        sequence_output, visual_output = sequence_output.contiguous(), visual_output.contiguous()
        """
        sim_header is seqTransf. Others are not used.
        """
        if sim_header == "meanP":
            pass
            ...
            query_mask = torch.ones(video_mask.shape[0], query_embed.shape[1]).cuda(device=visual_output.device)
            ...
            audio_mask = torch.ones(video_mask.shape[0], qa_output.shape[1]).cuda(device=visual_output.device).contiguous()
        ...
        sim_l_fuse = (_LSE_real(torch.matmul(sequence_output_.unsqueeze(1), av_fused_output.transpose(1,2).unsqueeze(0)).squeeze(2) * logit_scale, lamb, video_mask, d=-1)/lamb)
        sim_g_fuse = torch.matmul(sequence_output_.squeeze(1), av_fused_output_global.t()) * logit_scale
        
        retrieve_logits = (sim_l_fuse + sim_g_fuse) / 2.0
        if self.training:
            beta = self.task_config.beta ### Lambda in the paper
            margin = self.task_config.margin_BD ### Delta in the paper
            with torch.no_grad():
                sim_tt = torch.matmul(sequence_output_.squeeze(1), sequence_output_.squeeze(1).t())
                sim_vv = torch.matmul(visual_output_global, visual_output_global.t())
    ...
    def get_similarity_logits(self, sequence_output, visual_output, audio_output, attention_mask, video_mask, shaped=False, loose_type=False):
        if shaped is False:
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
        ...
        if loose_type:
            ...
            if self.training:
                retrieve_logits = self._loose_similarity_audio_group(sequence_output, visual_output, audio_output, attention_mask, video_mask, sim_header=self.sim_header)
            else:
                retrieve_logits,gate_tuple = self._loose_similarity_audio_group(sequence_output, visual_output, audio_output, attention_mask, video_mask, sim_header=self.sim_header)
        else:
        ...
        if self.training:
            ...
            return retrieve_logits, gate_tuple, contrastive_direction
