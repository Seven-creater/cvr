...
if [ -n "$DATASET_VIDEO_ROOTS" ]; then
  IFS=';' read -r -a dataset_video_root_items <<< "$DATASET_VIDEO_ROOTS"
  for item in "${dataset_video_root_items[@]}"; do
    item="${item#"${item%%[![:space:]]*}"}"
    item="${item%"${item##*[![:space:]]}"}"
    test -n "$item" && args+=(--dataset-video-root "$item")
  done
fi
if [ "$DRY_RUN" = "1" ]; then
  args+=(--dry-run)
