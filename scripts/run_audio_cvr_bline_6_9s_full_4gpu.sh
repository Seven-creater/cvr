...
RUN_ROOT=${RUN_ROOT:-$REPO_ROOT/runs/audio_cvr_bline_6_9s_full_$(date +%Y%m%d_%H%M%S)}
...
usage() {
  cat <<'EOF'
Usage: run_audio_cvr_bline_6_9s_full_4gpu.sh [options]

Large-scale B-line Audio-CVR construction over clips/audio_cvr_6_9s.
This script runs B-line only with b_audio_blind_review_v2.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help) usage; exit 0 ;;
    *) echo "[audio-cvr-6-9s-full] unknown argument: $1" >&2; usage >&2; exit 2 ;;
  esac
done
...
stop_vllm() {
  ...
  echo "[audio-cvr-6-9s-full] stopping existing vllm pids=$pids"
...
start_vllm() {
  local log_path
  test -f "$MODEL_PATH/config.json" || { echo "[audio-cvr-6-9s-full] missing model config: $MODEL_PATH/config.json" >&2; exit 2; }
  log_path="$REPO_ROOT/logs/qwen3_omni_${PORT}_4gpu_$(date +%Y%m%d_%H%M%S).log"
  echo "[audio-cvr-6-9s-full] starting vllm log=$log_path gpu_ids=$GPU_IDS max_model_len=$MAX_MODEL_LEN max_num_seqs=$MAX_NUM_SEQS"
  ...
  echo "[audio-cvr-6-9s-full] vllm_log=$log_path"
}
...
wait_for_vllm() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    if service_healthy; then
      echo "[audio-cvr-6-9s-full] vllm health check passed"
      return
    fi
    sleep 10
  done
  echo "[audio-cvr-6-9s-full] vllm did not become healthy within ${VLLM_HEALTH_TIMEOUT_SECONDS}s" >&2
  exit 1
}
...
case "$START_OMNI" in
  auto)
    if service_healthy; then
      echo "[audio-cvr-6-9s-full] existing vllm service is healthy"
    else
      stop_vllm
      start_vllm
      wait_for_vllm
  ...
  always)
    ...
    ;;
  never)
    service_healthy || { echo "[audio-cvr-6-9s-full] vllm service is not healthy and --start-omni never was set" >&2; exit 1; }
    ;;
  *) echo "[audio-cvr-6-9s-full] invalid --start-omni: $START_OMNI" >&2; exit 2 ;;
esac

echo "[audio-cvr-6-9s-full] run_root=$RUN_ROOT"
echo "[audio-cvr-6-9s-full] single_source_root=$SINGLE_SOURCE_ROOT"
echo "[audio-cvr-6-9s-full] clip_seconds=$CLIP_SECONDS min=$MIN_CLIP_SECONDS max=$MAX_CLIP_SECONDS"
echo "[audio-cvr-6-9s-full] concurrency=$CONCURRENCY propose_parallel_jobs=$PROPOSE_PARALLEL_JOBS propose_shards=$PROPOSE_SHARDS"
